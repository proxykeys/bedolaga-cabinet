...
| INCY one-tap import | `de53d50a` + bot custom.patch | 2026-08-30: кнопка «Добавить подписку» для INCY в кабинете — `subscriptionLink` с `{{INCY_CRYPT1_LINK}}` (инжектится ботом в `/app-config`, т.к. панель v7.2.6 не резолвит шаблон и на sub-странице кнопка была бы битой); кабинет резолвит клиентcайд `@incy/link-encoder/sync` (sync — iOS Safari tap-tick) → `incy://crypt1/<payload>` → авто-импорт. На sub-странице — copyButton fallback. Bot: `_inject_incy_crypt1_button` в status.py (idempotent). При апгрейде панели — перенести кнопку в конфиг, инжекцию удалить. См. ProxyBook/BedolagaSubPageClients.md |
