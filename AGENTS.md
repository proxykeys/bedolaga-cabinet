...
| `app/localization/locales/{ru,en,fa,zh,ua}.json` | SUBSCRIPTION_*_TEMPLATE без traffic/servers; ключи `CHGDEV_*` (степпер устройств, ru/en); `AUTOPAY_BUTTON` → «💳 Автопродление», `CHANGE_DEVICES_BUTTON` → «📱 Изменить лимит устройств» (renamed 2026-08-24; полное «…количество…» не влезало в кнопку; дефолты в inline.py и menu_layout/constants.py синхронны) |
