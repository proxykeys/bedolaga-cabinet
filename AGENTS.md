...
| Hide gift button | bot custom.patch | 2026-08-31: кнопка «🎁 Подарить подписку» скрыта в боте — upstream-хардкод `gift_enabled = True` (purchase.py, my_subscriptions.py) игнорировал настройку `CABINET_GIFT_ENABLED=false` (кнопка вела в тупик с алертом «недоступна»). Патч: `gift_enabled = await is_gift_enabled(db)` + импорт. Фича не удалена: вернуть — включить флаг в админке кабинета. Грабли: в purchase.py якорь импорта admin_notification_service встречается дважды — вставлять только после первого вхождения. См. ProxyBook/BedolagaHideGiftButton.md |
| Bot autopay status + purchase consent | bot custom.patch + `cc5f1bee` | 2026-08-31: (A) строка «💳 Автопродление: включено/выключено» на экране «Подписка» бота (`purchase.py` + шаблоны ru.json; значения AUTOPAY_STATUS_* в ru.json поправлены на «включено/выключено» — дефолты autopay.py уже были такими); (B) при покупке в боте toggle «🔁 Автопродление» под степпером устройств (**default OFF**, callback `buypay:`), consent-гейт в `confirm_tariff_purchase` срабатывает при ЛЮБОЙ покупке (симметрия веб-чекбоксу f9395e9d): OFF → экран «оферта only» (`BUYPAY_CONSENT_TEXT_OFF`), ON → «оферта + рекуррентные» (URL-кнопки, callback `buypay_consent:` → повторный вызов confirm), после оплаты `update_subscription_autopay(True)` при ON; FSM `purchase_autopay`/`purchase_consent`, toggle сбрасывает consent; (C) веб-форма покупки: дефолт toggle ON → OFF (`cc5f1bee`) — автопродление осознанный opt-in на обеих поверхностях. Маркер источника покупки в транзакциях: бот без кавычек `Покупка тарифа {name}`, веб с кавычками `'{name}'`. См. ProxyBook/BedolagaBotAutopayPurchaseConsent.md |
...
| Copy-link fallback all apps | — (panel DB) | 2026-09-02: fallback «Скопировать ссылку» + путь ручной вставки в блоках «Добавление подписки» всех приложений (кроме INCY-эталона и TV Happ — копирование на ТВ нереализуемо): Happ×5, V2rayTun×4, Shadowrocket×2, RabbitHole×2, Koala×3 — единый текст «Если кнопка не сработала — нажмите «Скопировать ссылку» и вставьте её вручную: <путь>» + кнопка `copyButton` после `subscriptionLink`. Заодно исправлены битые тексты с отсылкой к «кнопке Получить ссылку в правом верхнем углу» (V2rayTun/RH добавление, Koala известные проблемы) — в хедере sub page только QR-значок без подписи, в кабинете хедера нет. Код кабинета/sub page не менялся (copyButton рендерится обеими поверхностями; в кабинете скрывается при `hide_link`). Верификация: GET by-uuid 21 copyButton; Playwright на живой sub-странице по всем 5 приложениям. Скрипт/бэкапы: `ProxyBook/backups/{add-copy-fallback.py, subpage-config-before-copy-fallback-2026-09-02.json, subpage-config-applied-2026-09-02-copyfallback.json}`. См. ProxyBook/BedolagaSubPageClients.md |
| Download buttons unify | — (panel DB) | 2026-09-04: унификация подписей кнопок скачивания в конфиге sub page — «Открыть в App Store/Google Play», «Скачать установщик (x64 / Apple Silicon / Intel / Universal / x64 .deb / …)», «Скачать APK». Исправлены битые ссылки: Koala ×4 (несуществующие ассеты → `Koala.Clash_arm64.pkg`/`Koala.Clash_x64.pkg`/`Koala.Clash_x86_64.rpm`/`Koala.Clash_aarch64.rpm`), Happ RU ios+macos (листинг «Plus» удалён РКН 03.2026 → новый официальный `id6783623643` по happ.su). Happ macOS: добавлена кнопка universal-dmg. INCY iOS: копипаст с android (Google Play+APK) → «Открыть в App Store» `id6756943388` + описание. V2rayTun удалён из App Store глобально → карточка убрана из ios/macos (android/windows остались; вернуть при восстановлении в сторе). Код не менялся. Верификация: все 34 ссылки HEAD 200/обоснованы, GET by-uuid == трансформ, Playwright по iOS/macOS/Windows (перехват window.open — кнопки без href). См. ProxyBook/BedolagaSubPageClients.md |
...
| Buy CTA wrap mobile | `aff67d5c` | Перенос подписи кнопки «Оформить подписку» в сетке StatsGrid на мобильных: `truncate` → `leading-snug` + `hyphens-auto` — на 375px текст обрезался ellipsis (ячейка grid-cols-2, после иконки/шеврона ~70px); теперь 2 строки на 360-390px, переносы по слогам на 320px (lang="ru"), высоты ячеек синхронны (grid stretch) |
| Store links RU cleanup | — (panel DB) | 2026-09-05: чистка стор-ссылок sub page после РКН-удалений. Happ RU `id6783623643` (ios+macos) мёртв ГЛОБАЛЬНО (404 во всех сторфронтах; переиздания Happ умирают после каждого требования РКН: 03.2026 оригинал, 06.2026 Plus `id6746188973`) → RU-кнопки удалены; Happ Global `id6504287215` в RU-сторфронте 404 → добавлена кнопка «Инструкция: смена региона App Store» → `support.apple.com/ru-ru/118283` («Изменение страны или региона для Аккаунта Apple»; НЕ 108318 — HT207594 редиректит на статью про iCloud+, проверять og:title) + подсказка в description (ios «…или используйте другое приложение из списка», macos «…или скачайте установщик напрямую»); android/V2rayTun + «Скачать APK» → `storage.v2raytun.com/v2RayTun.apk` (официальный, найден 2026-09-05). INCY/Shadowrocket/RabbitHole/Happ TV живы в RU — не тронуты; Google Play все живы. TestFlight отклонён (Rus-бета мертва, Global — лимит 10k). Код кабинета/sub page не менялся. Скрипт/бэкапы: `ProxyBook/backups/{store-links-cleanup.py, subpage-config-before-store-cleanup-2026-09-05.json, subpage-config-applied-2026-09-05-store-cleanup.json}`. См. ProxyBook/BedolagaSubPageClients.md |
| Account ID display + search | `76d57133` + bot custom.patch | 2026-09-01: идентификация юзера при обращении в поддержку (может быть только TG или только email → универсальный ID). Веб: карточка «Информация об аккаунте» — первой строкой `ID` (внутренний `user.id`, литерал-лейбл как соседний `Email`), строка «Telegram ID» скрыта при null (email-only); dev-preview зеркалит. Бот: ГЛАВНАЯ поверхность главного меню — rich-HTML (`MAIN_MENU_RICH_ENABLED=true` дефолт; билдер `app/utils/rich_menu.py::build_main_menu_rich_html`, общий для send/edit-входов) → ID-строка `🆔 <code>{user.id}</code>` добавлена ТУДА (после `<h4>👤 …</h4>`); `MAIN_MENU`-шаблон (5 локалей, kwarg в 2 живых call-sites menu.py) — лишь fallback при сбоях rich-рендера (первый деплой без rich-правки дал «id нигде нет»). `get_main_menu_text_simple` в start.py — мёртвый код, не патчен: оживить = KeyError по `{user_id}`. Бэкенд: `_user_search_conditions` (crud/user.py) цифровой запрос матчит и `User.id` (OR с telegram_id) — иначе показ ID бесполезен. E2E: rich-HTML юзера 18 (без подписок) содержит `🆔 <code>18</code>`, search='18' находит юзера 18, tg/username-поиск целы. См. ProxyBook/BedolagaAccountIdDisplay.md |
| Email dark theme | bot custom.patch | 2026-09-05: все письма `noreply@proxykeys.net` переведены с Bootstrap-blue на тёмную тему кабинета (фон `#141413`, карточка `#262624`, текст `#faf9f6`, монохромные кнопки как `.btn-primary`). Одна точка: переписан `_get_base_template` в `email_templates.py` (35 типов через него) + ~20 замен инлайн-цветов в телах (`#666`→`#9c9c94`, bootstrap-red→тинты error). Outlook-устойчивость: табличная вёрстка с `bgcolor`, inline-наследование цвета от ячейки контента, `_inline_critical_styles` дублирует кнопки/highlight инлайн (regex по `class="..."`, style-переопределения объединяются), `<meta color-scheme=dark>` против двойной инверсии Apple Mail. Хедер: лого PK 98×44 НАД карточкой на фоне страницы (header-таблица вне `.container`, зазор 20px, `display:block; margin:0 auto`) — `{CABINET_URL}/logo.png` (статичный ассет AppHeader `src/assets/brand/logo-dark.png` через `public/logo.png`, стабильный URL без хэша — коммит `5ce44f33`; НЕ branding-API `/api/cabinet/branding/logo` — тот ассет кабинет использует только как favicon; путь обязан быть публичным — без `/api` Caddy отдаёт SPA index.html, инцидент первого деплоя 2026-09-05) с wordmark-alt-фолбэком (блокировка картинок → стилизованный alt). Кнопки ВСЕ монохромные — upstream danger-оверрайд «Сбросить пароль» удалён, warning-текст нейтрализован (2026-09-06). Верхние углы карточки скруглены `border-radius: 16px 16px 0 0` на content-td (2026-09-08; bgcolor td рисует квадрат поверх радиуса таблицы). Токены захардкожены (не из `CABINET_THEME_COLORS`) — смена акцента в админке потребует правки констант. Верификация: рендер 7 типов + Playwright computed styles. Backup `custom-before-email-theme.patch`. См. ProxyBook/BedolagaEmailDarkTheme.md |
 | Winback discounts off | — (bot runtime settings) | 2026-09-08: отключены winback-волны автоскидок истёкшим подпискам (upstream-дефолт: wave2 10% на 2–4 день + wave3 20% на 5-й, обе `enabled: true` из коробки — на email-only юзеров уходили письма «Скидка 10% на продление» из `discount_offers`). Правка `data/notification_settings.json` (bind-mount, gitignored, НЕ в custom.patch) + restart; Day-1 напоминание без скидки осталось. Грабля: при потере `data/` дефолты вернутся молча — проверять файл при восстановлении. См. ProxyBook/BedolagaWinbackDiscounts.md |
| Email spacing + unsubscribe dark | bot custom.patch | 2026-09-18: (A) выравнивание отступов обёртки писем (`_get_base_template`): футер `padding: 20px 0 4px` → `20px 0 24px` (ссылка «Отписаться от рассылок» сидела в 4px от низа карточки), контент-td inline + CSS `.content` синхронно `16px 32px 8px` → `16px 32px 16px` (Outlook читает только inline — оба места обязательны); (B) страница отписки `/api/cabinet/public/unsubscribe` (`app/cabinet/routes/unsubscribe.py` — НОВЫЙ файл в custom.patch) переведена на токены email-темы: все 3 состояния (GET авто-форма, «Вы отписались» ok=True `#009e74`, «Ссылка не сработала» ok=False `#df615b`) — фон `#141413`, карточка `#262624` без бордера, текст `#faf9f6`, ссылка «Настройки уведомлений…» `#5baae6`, кнопка монохромная как `.button` писем. Логика GET/POST/RFC 8058 не тронута. 2026-09-18 (вечер): лого над карточкой — клон хедера писем (`_logo_html`, `{CABINET_URL}/logo.png` 98×44, зазор 20px, alt-wordmark-фолбэк; `12vh` перенесён с карточки на обёртку, cabinet_url пуст → лого пропускается). 2026-09-18 (ночь): `<link rel="icon">` на обеих страницах (`_favicon_html`) → `{CABINET_URL}/api/cabinet/branding/logo` (админский лого, как во вкладке кабинета; без него браузер показывал домен-кэш букву V; backup `custom-before-unsub-favicon.patch`). Грабля верификации: GET-страница авто-сабмитит POST — реальный токен юзера в браузере не открывать (реально отпишет); тестировать dummy-токеном + рендер функций в контейнере. Backup `custom-before-spacing-unsubscribe.patch`, лого — `custom-before-unsub-logo.patch`. См. ProxyBook/BedolagaEmailDarkTheme.md |
| Favicon preload fix | `04a5ece5` | 2026-09-18: фавикон на всех страницах показывал букву «V» вместо лого из админки — branding-лого читалось из in-memory blob в момент рендера; пока preload не done → null → letter-ветка (`logo_letter` в БД бота = «V»), а react-query structural sharing (refetch deep-equal кэшу) не давал ре-рендер. Фикс: новый `src/utils/earlyFavicon.ts` (bootstrap: кэш или публичный fetch branding → await preload → rounded PNG → setFavicon; покрывает /login и переходные страницы, где эффекта не было вовсе) + вызов в `main.tsx` рядом с initLogoPreload + favicon-эффекты `useBranding.ts`/`QuickPurchase.tsx` теперь await preloadLogo до letter-фолбэка. Хедер при этом НЕ менялся (там статический PK-ассет намеренно). **Rebase-заметка**: upstream починил это архитектурно (`fccc16b7` + #605, useDocumentBranding/brandMonogram) — при следующем rebase (upstream ушёл на 238 коммита, 1.67→1.78) наш коммит drop-нуть. Верификация: Playwright WebKit, свежая сессия /login → favicon = PNG лого. См. ProxyBook/BedolagaFaviconFix.md |
...
- **Расположение**: `/opt/remnawave/bedolaga/custom.patch` (3414 строк, 34 файла; `locales/` volume-mount файлы исключены)
...
| `app/handlers/subscription/tariff_purchase.py` | Device selector (`get_tariff_device_keyboard`, `format_device_purchase_preview`, `tariff_dev:` handler); single-tariff auto-select; «❌ Отмена» back buttons; fallback `BUY_TARIFF_BUTTON` → «📦 Оформить подписку»; автопродление при покупке (2026-08-31): toggle `buypay:` под степпером (default OFF, сбрасывает consent), consent-гейт в `confirm_tariff_purchase` (`_show_purchase_autopay_consent`, URL оферта+рекуррентные, `buypay_consent:` → повторный вызов confirm), `update_subscription_autopay(True)` после покупки при ON |
| `app/handlers/subscription/my_subscriptions.py` | Traffic display + кнопка [📊 Трафик] убраны; «Автоплатеж»→«Автопродление»; [🛒 Оформить подписку], [➕ Добавить устройства]; `gift_enabled = await is_gift_enabled(db)` (ветка multi-tariff, 2026-08-31) |
...
| `app/handlers/subscription/purchase.py` | No-manual-renewal guard; traffic/servers убраны из шаблонов; регистрация степпер-хендлеров `chgdev*`; регистрация `confirm_autopay_enable` (`autopay_enable_confirm`); `gift_enabled = await is_gift_enabled(db)` (скрытие кнопки подарка, 2026-08-31); строка «💳 Автопродление: включен/выключен» на экране «Подписка» (`autopay_status` в overview/daily-шаблонах, 2026-08-31) |
| `app/cabinet/services/email_templates.py` | 2026-09-05: тёмная тема писем — переписан `_get_base_template` (фон #141413, карточка #262624, монохромные кнопки, лого+alt-фолбэк, `<meta color-scheme=dark>`), `_inline_critical_styles` дублирует кнопки/highlight инлайн для Outlook; ~20 замен инлайн-цветов в телах. См. ProxyBook/BedolagaEmailDarkTheme.md |
| `app/cabinet/routes/admin_users.py` | 2026-09-01: reset-subscription → удаление панель-юзера вместо disable (mark_intentional_panel_deletion + `delete_remnawave_user`, fail-closed: панель не удалён → БД не трогаем; очистка `user.remnawave_id`; безусловно, вне зависимости от `deactivate_in_panel`) — иначе ночная синхронизация воскрешала зомби (см. ProxyBook/BedolagaAdminResetZombie.md). 2026-09-01 (вечер): reset-trial → сброс `has_had_paid_subscription`, когда после сброса подписок не осталось (multi-tariff-safe; reset-subscription флаг не трогает), честный message при skip + поле `skipped_active_paid` (см. ProxyBook/BedolagaTrialResetFlag.md) |
| `app/cabinet/schemas/users.py` | 2026-09-01: ResetTrialResponse + поле `skipped_active_paid: bool = False` — маркер skip для честного warning-тоста фронта (см. ProxyBook/BedolagaTrialResetFlag.md) |
| `app/services/remnawave_service.py` | 2026-09-01: guard в `_create_subscription_from_panel_data` + multi-tariff-ветке создания: не создавать подписку из не-ACTIVE панель-юзера (warning-лог; в лог-вызове `user_id=user.id`, НЕ telegram_id — MissingGreenlet на lazy-атрибутах) |
| `app/database/crud/user.py` | 2026-09-01: `_user_search_conditions` — цифровой поисковый запрос матчит и `User.id == N` (OR с telegram_id); внутренний ID показывается юзерам (профиль/бот) и должен быть находим. См. ProxyBook/BedolagaAccountIdDisplay.md |
| `app/handlers/menu.py` | 2026-09-01: `MAIN_MENU.format` (2 call-sites) передаёт `user_id=user.id` — строка `🆔 <code>{user_id}</code>` в fallback-меню (плейсхолдер в 5 локалях). Там же: no-manual-renewal guard `has_saved_cart=False` при активной подписке (resume-checkout недоступен активным). См. ProxyBook/BedolagaAccountIdDisplay.md |
| `app/utils/rich_menu.py` | 2026-09-01: `build_main_menu_rich_html` — ID-строка `🆔 <code>{user.id}</code>` после `<h4>👤 …</h4>` (rich-меню = основная поверхность главного меню, `MAIN_MENU_RICH_ENABLED=true` дефолт). См. ProxyBook/BedolagaAccountIdDisplay.md |
