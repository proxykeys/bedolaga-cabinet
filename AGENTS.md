...
| Switch = autopay style | `0733d0d9` | Примитив `Switch` (Radix) переведён на стиль toggle автопродления: трек `h-7 w-[52px]` (28×52), плоский белый бегунок 22px без тени (absolute left/top 3px + `translate-x-[23px]`), OFF `gray-400` (как TariffPurchaseForm), анимация 300ms. Цель — Профиль → Настройки уведомлений (4 тумблера); примитив кроме Profile используется только в dev-preview секциях. API (`checked`/`onCheckedChange`), haptic, focus-ring не изменились |
