...
| Account ID display + search | `76d57133` + bot custom.patch | 2026-09-01: идентификация юзера при обращении в поддержку (может быть только TG или только email → универсальный ID). Веб: карточка «Информация об аккаунте» — первой строкой `ID` (внутренний `user.id`, литерал-лейбл как соседний `Email`), строка «Telegram ID» скрыта при null (email-only); dev-preview зеркалит. Бот: `MAIN_MENU` (5 локалей) строка `🆔 <code>{user_id}</code>` под именем, kwarg в 2 живых call-sites menu.py (`get_main_menu_text_simple` в start.py — мёртвый код, не патчен: оживить = KeyError по `{user_id}`). Бэкенд: `_user_search_conditions` (crud/user.py) цифровой запрос матчит и `User.id` (OR с telegram_id) — иначе показ ID бесполезен. E2E: search='18' находит юзера 18, tg/username-поиск целы. См. ProxyBook/BedolagaAccountIdDisplay.md |
...
| `app/handlers/menu.py` | 2026-09-01: `MAIN_MENU.format` (2 call-sites) передаёт `user_id=user.id` — строка `🆔 <code>{user_id}</code>` в главном меню (плейсхолдер в 5 локалях). Там же: no-manual-renewal guard `has_saved_cart=False` при активной подписке (resume-checkout недоступен активным). См. ProxyBook/BedolagaAccountIdDisplay.md |
