...
| Switch = autopay style | `0733d0d9` + `efae677d` | Примитив `Switch` (Radix) переведён на стиль toggle автопродления: трек `h-7 w-[52px]` (28×52), плоский белый бегунок 22px без тени (absolute left/top 3px + `translate-x-[23px]`), OFF `gray-500` (2026-08-29 затемнён с gray-400 по просьбе), анимация 300ms. Цель — Профиль → Настройки уведомлений (4 тумблера); примитив кроме Profile используется только в dev-preview секциях. API (`checked`/`onCheckedChange`), haptic, focus-ring не изменились |
